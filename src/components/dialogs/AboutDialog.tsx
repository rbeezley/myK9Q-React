...
export const AboutDialog: React.FC<AboutDialogProps> = ({
  ...
  const [updateStatus, setUpdateStatus] = useState<'idle' | 'checking' | 'up-to-date' | 'update-available' | 'updating'>('idle');
  const statusRef = useRef(updateStatus);
  useEffect(() => { statusRef.current = updateStatus; }, [updateStatus]);
  ...
  useEffect(() => {
    ...
    const checkWaiting = async () => {
      try {
        ...
        // Ignore — will check manually
      }
    };
    checkWaiting();

    // Reset when dialog closes
    return () => { setUpdateStatus('idle'); };
  }, [isOpen]);
  ...
  const applyUpdate = useCallback(async () => {
    ...
    try {
      const registration = await navigator.serviceWorker.getRegistration();
      if (!registration?.waiting) {
        // No waiting worker — already up to date
  ...
  const checkForUpdates = useCallback(async () => {
    ...
    try {
      ...
      // Track whether we find an update via the installing worker
      ...
      const trackInstalling = (sw: ServiceWorker) => {
        ...
        // If it's already installed (unlikely but possible), handle immediately
        if (sw.state === 'installed') {
          setUpdateStatus('update-available');
        } else {
      ...
      // If there's already an installing SW, track it
      ...
      // Give time for updatefound event to fire, then show result
      setTimeout(() => {
        registration.removeEventListener('updatefound', onUpdateFound);
        if (!foundUpdate) {
          setUpdateStatus('up-to-date');
            ...
            {'serviceWorker' in navigator && (
              <button
                ...
                style={{
                  ...
                  fontSize: '0.8125rem',
                  fontWeight: 500,
                  color: updateStatus === 'up-to-date' ? 'var(--status-qualified)' :
                         updateStatus === 'update-available' ? 'var(--primary)' :
                         updateStatus === 'updating' ? 'var(--primary)' :
